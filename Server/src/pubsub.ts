...
import { getEnvironmentVariable } from "@openbook/common";
...
function shouldUpdateLine() {
  const updateProbability = 0.2;
  return Math.random() < updateProbability;
}

export function makeChannelName(messageType: string): string {
  return getEnvironmentVariable("REDIS_CHANNEL") + "_" + messageType;
}

function publish(buttonId, newValue) {
  //console.log(`Publishing buttonId ${buttonId} with value ${newValue}`);
  pubsub.publish(makeChannelName("LINE_UPDATE"), {
    lineUpdate: {
      buttonId: buttonId,
      newValue: newValue,
    },
