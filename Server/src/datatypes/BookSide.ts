...
import { determineWager, determineWin } from "@openbook/common";
...
class BookEntry {
  ...
  getTotal(): DollarAmount {
    ...
    result.add(
      new DollarAmount(determineWin(this.line.getAsLegacyLine(), this.wager))
    );
    return result;
  }
...
class BookLevel {
  ...
  addBet(betId: bigint, lineDollarAmountPair: LineDollarAmountPair) {
    this.entries.push(
      new BookEntry(
        betId,
        lineDollarAmountPair.dollarAmount,
        lineDollarAmountPair.line
      )
    );
  }

  match(
    aggressingBetId: bigint,
    aggressingSide: Side,
    aggressingWager: LineDollarAmountPair
  ): Array<Match> {
    ...
    for (let i = 0; i < this.entries.length; i++) {
      ...
      const aggressingDesiredWin = new DollarAmount(
        determineWager(
          entry.line.getAsLegacyLine(),
          aggressingWager.dollarAmount.value
        )
      );
      ...
      if (
        aggressingDesiredWin.isGreater(entry.wager) ||
        aggressingDesiredWin.isEqual(entry.wager)
      ) {
        matchedPassiveWager = new DollarAmount(entry.wager.value);
        matchedAggressiveWager = new DollarAmount(
          determineWin(entry.line.getAsLegacyLine(), entry.wager.value)
        );
      ...
      matches.push(
        new Match(
          ...
          entry.betId,
          aggressingSide,
          new LineDollarAmountPair(
            aggressingWager.line,
            matchedAggressiveWager
          ),
...
export abstract class BookSide {
  ...
  abstract addBet(
    betId: bigint,
    lineDollarAmountPair: LineDollarAmountPair
  ): void;
  abstract tryMatch(
    aggressingBetId: bigint,
    aggressingWager: LineDollarAmountPair
...
export class ExplicitBookSide extends BookSide {
  ...
  addBet(betId: bigint, lineDollarAmountPair: LineDollarAmountPair) {
    ...
    for (i = 0; i < this.levels.length; i++) {
      if (lineDollarAmountPair.line.isEqual(this.levels[i].line)) {
        ...
        lineDollarAmountPair.line.isMoreAggressive(
          this.side,
          this.levels[i].line
        )
      ) {
  ...
  tryMatch(
    ...
    for (let i = 0; i < this.levels.length; i++) {
      ...
      if (
        level.line.canMatchAgainst(
          sideFlip(this.side) /* aggressing side */,
          aggressingWager.line
        )
      ) {
        const levelMatches = level.match(
          aggressingBetId,
          sideFlip(this.side),
          aggressingWager
        );
...
export class ImplicitOutrightBookSide extends BookSide {
  ...
  constructor(side: Side, implicativeBookSides: Array<BookSide>) {
    ...
    this.implicativeBookSides.forEach((implicativeBookSide) => {
      assert(
        implicativeBookSide.side != side,
        `A ${implicativeBookSide.side} book cannot imply into a ${side} book`
      );
    });
  }
  ...
  getLevels(maxDepth: number = 10): Array<LineDollarAmountPair> {
    ...
    while (impliedBookLevels.length < maxDepth) {
      ...
      for (let bookIndex = 0; bookIndex < numBooks; bookIndex++) {
        topImplicativeBookLevels.push(
          implicativeBookLevels[bookIndex][exhaustedBookDepths[bookIndex]]
        );
      }
      ...
      let minImplicativeDollarAmount = new DollarAmount(
        Number.MAX_SAFE_INTEGER
      );
      for (let bookIndex = 0; bookIndex < numBooks; bookIndex++) {
        // check if implicativeBookLevels[bookIndex] is exhausted
        if (
          exhaustedBookDepths[bookIndex] ==
          implicativeBookLevels[bookIndex].length
        ) {
          continue;
        }

        cumulativeOpposingProbability += convertLineToProbability(
          topImplicativeBookLevels[bookIndex].line.getAsUnscaledFloat()
        );
        if (
          topImplicativeBookLevels[bookIndex].dollarAmount.isLess(
            minImplicativeDollarAmount
          )
        ) {
          minImplicativeDollarAmount = cloneable.deepCopy(
            topImplicativeBookLevels[bookIndex].dollarAmount
          );
        }
      }
      ...
      for (let bookIndex = 0; bookIndex < numBooks; bookIndex++) {
        ...
        if (
          topImplicativeBookLevels[bookIndex].dollarAmount.isEqual(
            minImplicativeDollarAmount
          )
        ) {
          exhaustedBookDepths[bookIndex] += 1;
        }
        topImplicativeBookLevels[bookIndex].dollarAmount.subtract(
          minImplicativeDollarAmount
        );
      }
      ...
      const impliedLine = MoneyLine.RoundToLessAggressive(
        unscaledLine,
        this.side
      );
      // If the user bidding into the implied book wins, they will win the minImplicativeDollarAmount
      const impliedAvailableWin =
        minImplicativeDollarAmount.value * numNonExhaustedBooks;
      const impliedAvailableWager = determineWager(
        { type: "MoneyLine", value: impliedLine.getAsUnscaledFloat() },
        impliedAvailableWin
      );
      ...
      if (
        impliedBookLevels.length > 0 &&
        impliedBookLevels[impliedBookLevels.length - 1].line.isEqual(
          impliedLine
        )
      ) {
        impliedBookLevels[implicativeBookLevels.length - 1].dollarAmount.add(
          impliedQty
        );
      } else {
        impliedBookLevels.push(
          new LineDollarAmountPair(impliedLine, impliedQty)
        );
      }
    }
