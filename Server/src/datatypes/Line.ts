...
export abstract class Line {
    ...
    canMatchAgainst(aggressingSide: Side, aggressingLine: Line): boolean {
        ...
        assert(false, `${this.getType()} cannot be matched against ${aggressingLine.getType()}`);
        return false;
    }

    isValid(): boolean {
        return Number.isInteger(this.value)
            && this.value % this.tickSize() == 0;
    }

    getType(): LineType {
        return this instanceof MoneyLine ? LineType.Money
            : this instanceof SpreadLine ? LineType.Spread
            : this instanceof TotalLine ? LineType.Total
            : LineType.None;
    }
    ...
    getAsLegacyLine() {
        return this.getType() == LineType.Money ? { type: "MoneyLine", value: this.getAsUnscaledFloat() }
            : this.getType() == LineType.Spread ? { type: "SpreadLine", value: this.getAsUnscaledFloat() }
            : this.getType() == LineType.Total ? { type: "TotalLine", value: this.getAsUnscaledFloat() }
            : {}
    }
...
export class MoneyLine extends Line {
    static RoundToLessAggressive(unscaledLine: number, side: Side) {
        ...
        if (scaledLine < 0)
            scaledLine = side == Side.Bid ? roundUp(scaledLine, MoneyLine.TickSize()) : roundDown(scaledLine, MoneyLine.TickSize());
        else
            scaledLine = side == Side.Bid ? roundDown(scaledLine, MoneyLine.TickSize()) : roundUp(scaledLine, MoneyLine.TickSize());
    ...
    isValid(): boolean {
        return (this.getAsUnscaledFloat() < -100 || this.getAsUnscaledFloat() >= 100)
            && super.isValid();
    }
    ...
    canMatchAgainst(aggressingSide: Side, aggressingLine: Line): boolean {
        ...
        let canMatch = this.getAsUnscaledFloat() <= -100 && aggressingLine.getAsUnscaledFloat() <= -100
            ...
            || this.getAsUnscaledFloat() == -100 && aggressingLine.getAsUnscaledFloat() == 100
            || this.getAsUnscaledFloat() == 100 && aggressingLine.getAsUnscaledFloat() == -100
            || this.value < 0 && aggressingLine.value > 0 && -1 * this.value >= aggressingLine.value
            || this.value > 0 && aggressingLine.value < 0 && -1 * aggressingLine.value >= this.value;
        return canMatch;
    }
}
...
export class TotalLine extends Line {
    ...
    canMatchAgainst(aggressingSide: Side, aggressingLine: Line): boolean {
        ...
        let canMatch = aggressingSide == Side.Ask && aggressingLine.value <= this.value
